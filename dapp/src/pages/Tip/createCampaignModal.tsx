...
export const CreateCampaignModal: FC<Props> = ({ closeModal }) => {
    ...
    return (
        <div>
            <div className="text-2xl mb-4">{t('tip.create_modal.title')}</div>

            <form onSubmit={handleSubmit(submit)}>

                <div className="flex flex-col gap-y-4 gap-x-6 mb-4">
                    <div {...getRootProps()} className="relative border rounded-md col-span-3 h-30 md:h-60 flex flex-col justify-center items-center gap-6 cursor-pointer p-2 md:p-0">
                        { file ? (
                            <>
                            ...
                            <>
                                ...
                                {
                                    isDragActive ?
                                        <p className='text-sm md:text-base text-gray-500'>{t('tip.create_modal.drop_files')}</p> :
                                        <p className='text-sm md:text-base text-gray-500'>{t('tip.create_modal.drag_files')}</p>
                                }
                            </>
                        )}
                    </div>
                    ...
                    <div>
                        <div className="flex flex-col md:flex-row items-stretch gap-6">
                            <div className='flex-auto'>
                                <label className="block text-sm font-medium text-gray-700">{t('tip.create_modal.name.label')}</label>
                                <input {...register('title', { required: true })} type="text"
                                    className="border p-2 w-full rounded-md border-gray-300 leading-5"
                                    placeholder={t('tip.create_modal.name.placeholder')} />
                                {errors.title?.type === 'required' && (<div className="text-red-400">{t('tip.create_modal.name.errors.required')}</div>)}
                            </div>

                            <div className='flex-shrink-0 md:w-48'>
                                <label className="block text-sm font-medium text-gray-700">{t('tip.create_modal.currency.label')}</label>
                    ...
                    <div>
                        <label className="block text-sm font-medium text-gray-700">{t('tip.create_modal.description.label')}</label>
                        <textarea {...register('description', { required: true })}
                            className="h-32 border p-2 w-full rounded-md border-gray-300"
                            placeholder={t('tip.create_modal.description.placeholder')} />
                        {errors.description?.type === 'required' && (<div className="text-red-400">{t('tip.create_modal.description.errors.required')}</div>)}
                    </div>

                </div>
                <div className="flex flex-col-reverse md:flex-row justify-end items-center gap-6">
                    <div className="text-xs text-gray-500 w-64">
                        {t('tip.create_modal.fee_notice')}
                    </div>
                    <button type='submit' className={primaryButton}>
                    { submitting ?
                            <LoaderIcon className='w-4 h-6' /> :
                            <>{t('tip.create_modal.create')}</>
                        }
                    </button>
                </div>
            </form>
            ...
            <div className="text-xs text-gray-500 mt-2 md:mt-10">
                <Trans i18nKey='tip.create_modal.data_notice'>
                    ...
                    <br />Please make sure not to send sensible data.
                </Trans>
            </div>
        </div>
    );
